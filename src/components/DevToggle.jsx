...
/**
 * DevToggle — Selector rápido de roles (solo en desarrollo).
 * Desaparece automáticamente en producción (import.meta.env.PROD).
 * Estilo: Burbuja Mágica flotante con degradado neón.
 */
export const DevToggle = () => {
  const { rolActual, setRolActual } = useAuth();

  if (import.meta.env.PROD || IS_REAL_AUTH) return null;

  return (
    <div className="fixed bottom-24 right-4 md:bottom-6 md:right-6 z-[9999] group">
      {/* Halo de color */}
      <div className="absolute inset-0 bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 rounded-2xl blur opacity-75 group-hover:opacity-100 transition duration-300" />
